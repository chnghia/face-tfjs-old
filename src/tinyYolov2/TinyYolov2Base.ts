...
export class TinyYolov2Base extends NeuralNetwork<TinyYolov2NetParams> {
  ...
  public async detect(input: TNetInput, forwardParams: ITinyYolov2Options = {}): Promise<ObjectDetection[]> {
    ...
    const boxes = results.map((res: any) => res.box);
    const scores = results.map((res: any) => res.score);
    const classScores = results.map((res: any) => res.classScore);
    const classNames = results.map((res: any) => this.config.classes[res.label]);

    const indices = nonMaxSuppression(
      boxes.map((box: any) => box.rescale(inputSize)),
      scores,
